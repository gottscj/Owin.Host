...
namespace Gottscj.Owin.Host.SocketHttpListener
{
    internal class WebSocketFrame : IEnumerable<byte>
    {
        ...
        {
            _fin = fin;
            _rsv1 = IsDataInternal(opcode) && compressed ? Rsv.On : Rsv.Off;
            _rsv2 = Rsv.Off;
            _rsv3 = Rsv.Off;
            _opcode = opcode;
            _mask = mask;
        ...
        private static bool IsControlInternal(Opcode opcode)
        {
            return opcode == Opcode.Close || opcode == Opcode.Ping || opcode == Opcode.Pong;
        }

        private static bool IsDataInternal(Opcode opcode)
        {
        ...
        private static WebSocketFrame Read(byte[] header, Stream stream, bool unmask)
        {
            ...
            var incorrect = IsControlInternal(opcode) && fin == Fin.More
                            ? "A control frame is fragmented."
                            : !IsDataInternal(opcode) && rsv1 == Rsv.On
            ...
            if (IsControlInternal(opcode) && payloadLen > 125)
